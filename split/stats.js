...
/* Aggregate tonnage, carrying the share of lines it could actually be
   established for. A tonnage figure quoted without its coverage is a figure
   that quietly understates itself whenever a part has no weight on file. */
function weighLines(rows) {
  var kg = 0, lines = 0, known = 0;
  rows.forEach(function(row) {
    (row.items || []).forEach(function(it) {
      lines++;
      var w = lineWeightKg(it);
      if (w.known) { known++; kg += w.kg; }
    });
  });
  return { kg: kg, lines: lines, known: known, coverage: lines > 0 ? known / lines : 1 };
}
...
function buildClientRollup(invoices) {
  ...
  invoices.forEach(function(inv) {
    ...
    if (!by[key]) {
      by[key] = { clientId: key, name: inv.clientName, total: 0, count: 0, kg: 0, kgKnown: true };
    }
    by[key].total += (inv.taxableValue || 0);
    by[key].count++;
    (inv.items || []).forEach(function(it) {
      var w = lineWeightKg(it);
      if (w.known) by[key].kg += w.kg; else by[key].kgKnown = false;
    });
  });
  return Object.values(by).map(function(r) {
    r.realisation = r.kg > 0 ? r.total / r.kg : null;
    return r;
...
function renderStats() {
  ...
  var realisation = tonnage.kg > 0 ? totalRev / tonnage.kg : null;
  var priorRealisation = priorTonnage.kg > 0 ? priorRev / priorTonnage.kg : null;
  ...
  // Tonnage is only ever as good as the weights behind it. Say so, in place,
  // rather than letting an understated figure pass as complete.
  if (tonnage.lines > 0 && tonnage.coverage < 1) {
    var missing = tonnage.lines - tonnage.known;
    html += '<div class="inv-stats-caveat">Tonnage covers ' + Math.round(tonnage.coverage * 100) + '% of lines &mdash; ' +
      missing + ' line' + (missing === 1 ? '' : 's') + ' priced in NOS with no weight on file, so the real tonnage is higher ' +
      'and realisation is overstated. Fill weights in Items Master to close the gap.</div>';
  }
  ...
  if (ranked.length > 0 && tonnage.kg > 0) {
    var byRealisation = ranked.filter(function(r) { return r.realisation != null; })
      .sort(function(a, b) { return a.realisation - b.realisation; });
    if (byRealisation.length > 0) {
      html += '<div class="inv-stats-card inv-stats-card-full">' +
      ...
      byRealisation.forEach(function(r) {
        ...
        html += '<div class="inv-stats-table-row inv-stats-row-tap" data-action="invStatsClientDrill" data-client-id="' + r.clientId + '">' +
          ...
          (r.kgKnown ? '' : '<br><span class="inv-text-muted inv-text-xs">partial weights</span>') + '</span>' +
      ...
      html += '</div>';
      if (costPerKg > 0) {
        var belowCost = byRealisation.filter(function(r) { return r.realisation < costPerKg; });
  ...
  if (ranked.length > 1 && totalRev > 0) {
    ...
    var topKgShare = tonnage.kg > 0 ? (top.kg / tonnage.kg) * 100 : null;
    ...
    html += '<div class="inv-stats-card">' +
      ...
      (topKgShare != null ? '<div class="inv-stats-row"><span class="inv-stats-name">Share of tonnage</span>' +
        '<span class="inv-stats-val' + (topKgShare - topRevShare > 10 ? ' inv-stats-val-warn' : '') + '">' +
        formatNum(topKgShare, 0) + '%</span></div>' : '') +
...
function openClientDrillOverlay(clientId) {
  ...
  var realisation = clientTonnage.kg > 0 ? totalRev / clientTonnage.kg : null;
  var costPerKg = S.defaultCostPerKg || 0;
