...
function refreshZincRate() {
  ...
  fetch('https://api.metals.dev/v1/latest?api_key=' + encodeURIComponent(key) +
    ...
    .then(function(json) {
      ...
      // Record WHICH field the figure came from. An LME quote is ~16% below
      // MCX once duty and freight are counted, so a rate whose exchange is not
      // visible can be wrong by that much without anyone noticing.
      z.source = 'metals.dev · ' + hit.field;
      saveState();
      renderZincCard();
      showToast('Zinc ' + formatCurrency(zincLandedRate()) + '/kg landed (' + hit.field + ')');
    })
    .catch(function(err) {
...
/* Returns {rate, field} or null. Field names are tried MCX-first on purpose:
   this shop buys at the Indian exchange price, and an LME quote runs roughly
   16% below it once import duty and freight are counted. Taking whichever
   zinc figure appeared first would silently understate the largest bought-in
   input, so the matched field name is returned with the rate and shown in the
   UI — a wrong exchange becomes visible instead of quietly wrong. */
function _extractZincRate(json) {
  ...
  var names = ['mcx_zinc', 'zinc_mcx', 'zinc', 'Zinc', 'ZINC', 'lme_zinc', 'zinc_lme'];
