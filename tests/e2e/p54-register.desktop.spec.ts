...
for (const width of [1440, 1024]) {
  test(`at ${width}px the keyboard keeps its place when the table is redrawn`, async ({ page }) => {
    ...
    const first = page.locator('#regMaster button[data-invnum]').first();
    ...
    await page.keyboard.press('Enter');
    await expect(row(id)).toBeFocused();

    // A sort from the keyboard, and a tick box from the space bar, both keep focus.
    const sort = () => page.locator('#regMaster button[data-action="invDesktopSort"][data-col="total"]');
    await sort().focus();
    await page.keyboard.press('Enter');
    await expect(sort()).toBeFocused();
    const tick = () => page.locator(`#regMaster input[data-action="invRegToggleInv"][data-id="${id}"]`);
    await tick().focus();
...
test('a cancelled row fades once, not once per cell and again for the row', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  const s = state();
  (s.invoices as any)[0].status = 'cancelled';
  await loadAppWithState(page, s);
  await switchTab(page, 'pageRegister');
  const fade = await page.locator('#regMaster tr[data-cancelled]').evaluate((tr) => {
    const td = tr.querySelector('td')!;
    return Number(getComputedStyle(tr).opacity) * Number(getComputedStyle(td).opacity);
  });
  expect(fade).toBeGreaterThan(0.5);
});
