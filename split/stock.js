...
function resolveStockParse(parsed, choices) {
  ...
  parsed.lines.forEach(function(l, idx) {
    ...
    if (mapChoice && mapChoice !== 'new') { r.item = stockItem(mapChoice); r.via = 'chosen'; }
    ...
    if (r.via === 'new') {
      r.newItem = { name: stockDisplayName(l.key), key: l.key, unit: l.unit || '', basis: stockGuessBasis(l.key) };
...
function renderStockReview() {
  ...
  sorted.forEach(function(r) {
    ...
    if ((r.via === 'new' && st.items.length) || r.via === 'position' || r.via === 'chosen' || (r.skip && !r.item)) {
